...
parser.add_argument('--load_gt_flow', type=bool, default=True)#False
parser.add_argument('--load_1st_GS', type=int, default=1)
...
class Inference(Generic_train_test):
    ...
    def test(self):
        ...
        for i, data in enumerate(self.dataloader):
            ...
            if self.opts.load_1st_GS==1:
                with torch.no_grad():
                    pred_im, _, _, _, _ = self.model.forward(0.5)
            else:
                with torch.no_grad():
                    pred_im, _, _, _, _ = self.model.forward(1.0)
            torch.cuda.synchronize()
            time_end=time.time()
            ...
            if self.opts.compute_metrics:
                ...
                if self.opts.dataset_type=='Fastec':
                    predict_GS = F.interpolate(predict_GS, size=[480,640], mode='bilinear')#,align_corners=True  bicubic  bilinear
                    GT_GS = F.interpolate(GT_GS, size=[480,640], mode='bilinear')
